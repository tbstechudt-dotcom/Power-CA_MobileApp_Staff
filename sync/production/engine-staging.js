...
class StagingSyncEngine {
  ...
  async validateTimestampColumns() {
    try {
      console.log('[INFO] Validating timestamp columns for incremental sync...');
      ...
      if (missingTables.length > 0) {
        console.log(`[WARN]  WARNING: ${missingTables.length} table(s) missing timestamp columns:`);
        for (const { table, missing } of missingTables) {
          console.log(`   - ${table}: missing ${missing}`);
        }
        console.log(`[WARN]  These tables will use FULL sync instead of incremental\n`);
      } else {
        console.log(`[OK] All ${tables.length} tables have timestamp columns\n`);
      }

    } catch (error) {
      console.error('[WARN]  Error validating timestamp columns:', error.message);
   ...
   * Returns { hasCreatedAt, hasUpdatedAt, hasEither }
   */
  ...
  async syncTableSafe(sourceTableName, mode = 'full') {
    ...
    try {
      ...
      if (effectiveMode === 'incremental') {
        // Verify table has timestamp columns before using them for incremental sync
      ...
      try {
        ...
        // OPTIMIZED: Batch INSERT (1000 records per query instead of 1)
        ...
        for (let batchStart = 0; batchStart < validRecords.length; batchStart += BATCH_SIZE) {
          ...
          // Get columns from first record (all records have same structure)
          ...
          try {
            ...
            console.error(`    [X] Error loading batch to staging:`, error.message.split('\n')[0]);
            ...
            for (const record of batchRecords) {
              try {
                ...
                const recordPlaceholders = recordColumns.map((_, i) => `$${i + 1}`);
                await client.query(
                  `INSERT INTO ${stagingTableName} (${recordColumns.join(', ')}) VALUES (${recordPlaceholders.join(', ')})`,
                  recordValues
                );
                stagingLoaded++;
              } catch (rowError) {
                console.error(`    [X] Error loading record:`, rowError.message.split('\n')[0]);
              }
            }
          }
        }
