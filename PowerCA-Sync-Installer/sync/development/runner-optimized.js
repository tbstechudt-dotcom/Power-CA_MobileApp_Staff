#!/usr/bin/env node
/**
 * Optimized Sync Runner - Entry Point for Optimized Engine
 *
 * Uses FK pre-filtering for 40-60% performance improvement.
 *
 * Usage:
 *   node sync/runner-optimized.js --mode=full          # Full sync (optimized)
 *   node sync/runner-optimized.js --mode=incremental   # Incremental sync (optimized)
 *   node sync/runner-optimized.js --test               # Test connections only
 *   node sync/runner-optimized.js --table=jobshead     # Sync single table
 *   node sync/runner-optimized.js --dry-run            # Dry run (no changes)
 */

const OptimizedSyncEngine = require('./engine-optimized');
...
function showHelp() {
  console.log(`
Power CA → Supabase Sync Tool (OPTIMIZED)
==========================================

PERFORMANCE FEATURES:
  ✓ Pre-filters invalid FK records (40-60% faster)
  ✓ Batch inserts when possible (10x faster)
  ✓ Detailed filtering statistics
  ✓ Reduced network roundtrips

Usage:
  node sync/runner-optimized.js [options]
...
Examples:
  # Full sync of all tables (optimized)
  ...
  # Test connections
  node sync/runner-optimized.js --test
...
Performance Comparison:
  Standard Engine:  60+ minutes for full sync
  Optimized Engine: 20-30 minutes for full sync (estimated)
